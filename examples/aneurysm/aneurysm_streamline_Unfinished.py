...
#TODO: This file is unfinished and needs to be completed.
#還沒辦法抓到bounds_lower, bounds_upper，需再參考Limerock的做法
...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    # plot streamline 2025/8/3
    def read_solid(reader):
        faces = []
        while True:
            line = reader.readline()
            split_line = line.split()
            if len(split_line) == 0 or split_line[0] == "endsolid":
                break
            elif split_line[0] == "facet":
                _ = reader.readline()  # outer loop
                ...
                _ = reader.readline()  # endloop
                _ = reader.readline()  # endfacet
        if len(faces) == 0:
            raise ValueError("read_solid: No facets found in STL solid.")
        ...
        print(f"read_solid: found {len(faces)} facets")
        return bounds_lower, bounds_upper


    def parse_file(filename):
        with open(filename) as reader:
            geo_bounds_lower = None
            geo_bounds_upper = None
            while True:
                line = reader.readline()
                if not line:
                    break
                if "solid" == line.split()[0]:
                    bounds_lower, bounds_upper = read_solid(reader)
                    geo_bounds_lower = bounds_lower if geo_bounds_lower is None else tuple(min(x,y) for x,y in zip(geo_bounds_lower, bounds_lower))
                    geo_bounds_upper = bounds_upper if geo_bounds_upper is None else tuple(max(x,y) for x,y in zip(geo_bounds_upper, bounds_upper))
        if geo_bounds_lower is None or geo_bounds_upper is None:
            raise ValueError("parse_file: No solids found in STL file.")
        return {
            "geo_bounds_lower": geo_bounds_lower,
            "geo_bounds_upper": geo_bounds_upper,
        }

    # ---- 主程式流程 ----
    print("parsing stl file...")
    fluid_zone = parse_file(os.path.join(point_path, ""))
    print("finished parsing")

    geo_bounds_lower = fluid_zone["geo_bounds_lower"]
    geo_bounds_upper = fluid_zone["geo_bounds_upper"]

    fluid_zone["geo_bounds"] = {
        Symbol("x"): (geo_bounds_lower[0], geo_bounds_upper[0]),
        Symbol("y"): (geo_bounds_lower[1], geo_bounds_upper[1]),
        Symbol("z"): (geo_bounds_lower[2], geo_bounds_upper[2]),
    }
    ...
    vtk_obj = VTKUniformGrid(
        bounds=[fluid_zone["geo_bounds"][Symbol("x")],
                fluid_zone["geo_bounds"][Symbol("y")],
                fluid_zone["geo_bounds"][Symbol("z")]],
        npoints=[64, 64, 64],  # 減少點數測試
        export_map={"u": ["u", "v", "w"], "p": ["p"]},
    )
